...
class RepositoryContractTests(unittest.TestCase):
	...
	def test_deploy_is_persistent_and_restart_is_opt_in(self):
		...
		self.assertIn("repair_nginx_runtime_permissions", deploy)
		self.assertIn("sync_frontend_dependency_assets", deploy)
		self.assertIn("nginx -t", deploy)
		self.assertIn("nginx -s reload", deploy)
		self.assertIn('docker exec -u frappe "$FRONTEND_CONTAINER" nginx -t', deploy)
		self.assertIn('docker exec -u frappe "$FRONTEND_CONTAINER" nginx -s reload', deploy)
		self.assertNotIn('docker exec -u root "$FRONTEND_CONTAINER" nginx', deploy)
		self.assertIn("chown frappe:frappe", deploy)
		self.assertIn("for dir in body proxy fastcgi uwsgi scgi", deploy)
		self.assertIn(".frontend-$app-assets.", deploy)
		self.assertIn("for app in chat hrms", deploy)
