<template>
  <div class="shell">
    ...
    <main class="layout">
      ...
      <div v-if="error" class="error" role="alert">{{ error }} <a v-if="unauthenticated" href="/login?redirect-to=%2Fccd-portal">{{ __('Sign in') }}</a></div>
...
function setError(e) { error.value=e.message || __("The request could not be completed."); unauthenticated.value=e.status===401 || e.status===403; }
